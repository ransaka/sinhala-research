...
For an automated pinned download and run, use `scripts/run_slr52.sh`. It accepts `--gpus 2` for single-node CUDA training with `torchrun` and a frozen `--split` CSV or an explicit `--exploratory-split`. See the [runbook](docs/RUNNING_EXPERIMENTS.md) for exact commands.
