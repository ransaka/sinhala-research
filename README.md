...
To run all three transcript targets at the same budget, use `scripts/run_tokenizer_comparison.sh`. It trains SentencePiece unigram and sinlib tokenizers on the selected training transcripts from scratch. Each arm writes its own training log, metrics, decoded predictions, tokenizer assets, and resumable checkpoint. New checkpoints include the tokenizer assets and audio feature extractor beside the model weights for later loading.
